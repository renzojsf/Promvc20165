     ...
     <tr>
      <td align="center">
        ...
        <table border="0" align="center" width="730px" cellpadding="0" cellspacing="0">
         <tr>
          <td colspan="2">
			<? $_act=permite(3); $_lis=permite(4); $_rep=permite(49); ?>
           <form name="listado" id="listado" method="post">
             ...
             <tr>
              <td>
               <table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" style="border:solid 1px #7f7f7f;font-size:12px;font-family:verdana;text-shadow:-1px -1px white, 1px 1px #999;color:#333">
                <tr height="25px">
                 <? if($_act=='1'){ ?><td width="30px" align="center" class="cabeza">A</td><? } ?>
                 <? if($_lis=='1'){ ?><td width="30px" align="center" class="cabeza">M</td><? } ?>
                 <? if($_rep=='1'){ ?><td width="30px" align="center" class="cabeza">R</td><? } ?>
                 <td align="center" width="140px" class="cabeza">Codigo</td>
                 <td align="left" style="text-indent:10px" class="cabeza">Nombre del alumno</td>
            ...
            <tr>
             <td style="background-color:#<? echo $color; ?>">
              <table border="0" width="100%" cellpadding="0" cellspacing="0" align="center" style="border:dashed 1px #A4A4A4;font-size:12px;font-family:verdana">
               <tr height="30px" <? if(isset($_REQUEST['ref']) and $_REQUEST['ref']==$vector['idalumno']){ echo "style='background-color:#DDDDDD'";} ?>>
                <? if($_act=='1'){ ?><td width="30px" align="center"><img src="img/actualiza.png" onclick="document.location.href='principal.php?sis=<?=$_REQUEST['sis']?>&ven=<?=$_REQUEST['ven']?>&pag=3&id=<?=$vector['idalumno']?>';" style="cursor:pointer;" title="Actualizar datos"></td><? } ?>
                <? if($_lis=='1'){ ?><td width="30px" align="center"><img src="img/historia.png" onclick="document.location.href='principal.php?sis=<?=$_REQUEST['sis']?>&ven=<?=$_REQUEST['ven']?>&pag=4&id=<?=$vector['idalumno']?>';" style="cursor:pointer;" title="Matriculas"></td><? } ?>
                <? if($_rep=='1'){ ?><td width="30px" align="center"><img src="img/lnotas.png" onclick="document.location.href='principal.php?sis=<?=$_REQUEST['sis']?>&ven=<?=$_REQUEST['ven']?>&pag=49&id=<?=$vector['idalumno']?>';" style="cursor:pointer;" title="Ver reporte de notas por seccion"></td><? } ?>
            ...
            <? } if($numero==0){ ?><tr><td style="color:#060" align="center">No hay ningun alumno registrado.</td></tr><? } ?>
			<? if ($total_paginas > 1){ ?>
