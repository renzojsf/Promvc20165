...
<script>
function aigna_padre(idtrabajador,trabajador)
{
	window.opener.document.nuevo.tra.value = trabajador;
	window.opener.document.nuevo.htr.value = idtrabajador;
	self.close();
}
 ...
 <tr>
  <td colspan="2">
   <form name="listado" id="listado" method="post">
	...
	if($pagina-1>0){echo " <a href='listado_trabajador.php?pagina=".($pagina-1)."$criterio'>Ant</a>&nbsp;";}   
	for ($i=1;$i<=$total_paginas;$i++){if ($i == $pagina){echo "<b>$pagina</b>&nbsp;";}else{echo "<a href='listado_trabajador.php?pagina=".$i."$criterio'>".$i."</a>&nbsp;";}}
	if($pagina + 1 <= $total_paginas){echo "&nbsp;<a href='listado_trabajador.php?pagina=".($pagina+1)."$criterio'>Sig</a>";}  
       ?>
      </td>
     </tr> 
    <? } ?>
