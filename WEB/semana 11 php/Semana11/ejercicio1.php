...
<!-- fwtable fwsrc="diseños.png" fwpage="plantilla" fwbase="plantilla.jpg" fwstyle="Dreamweaver" fwdocid = "119485501" fwnested="0" -->
  ...
  <tr>
   ...
   <?php
   echo "<h2>Ejercicio 1 : Crear bd y tabla</h2>";
   //Crear la conexion
   $conec=mysql_connect("localhost","root","123");
   if(mysql_query("create database if not exists bdprueba",$conec))
   {
   		echo "<h2>Base de Datos Creada</h2>";
		mysql_select_db("bdprueba",$conec);//Seleccionar db
		$crear="create table if not exists clientes";
		$crear.="(codigo tinyint unsigned, nombre char(40) not null, ";
		$crear.="direccion char(60) not null)";
		if(mysql_query($crear,$conec))
		{
			echo "<h2>Tabla Creada</h2>";
		}
		mysql_close($conec);//cerrar la conexion.
   }
