...
<!-- fwtable fwsrc="diseños.png" fwpage="plantilla" fwbase="plantilla.jpg" fwstyle="Dreamweaver" fwdocid = "119485501" fwnested="0" -->
  ...
  <tr>
   ...
   mysql_select_db("bdprueba",$conec);
   
   mysql_query("insert into clientes values(1,'Fernando Quiroz','Av. Pardo 2514')",$conec);
   mysql_query("insert into clientes values(2,'Marcela','Jr. Paruro 514')",$conec);
   if(mysql_errno($conec)==0)
   {
   		echo "<h2>Registro Añadido</h2>";
   }
   else
   {
   $numerror=mysql_errno($conec);
   $deserror=mysql_error($conec);
   echo "<h2>Error Producido Nro: $numerror, $deserror.</h2>";
   }
